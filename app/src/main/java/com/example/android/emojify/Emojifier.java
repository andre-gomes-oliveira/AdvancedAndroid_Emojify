...
class Emojifier {
    ...
    /* Done by checking the provided example */
    ...
    private static Bitmap addBitmapToFace(Bitmap background, Bitmap emoji, Face face) {
        Bitmap resultBitmap = Bitmap.createBitmap(background.getWidth(),
                background.getHeight(), background.getConfig());

        float scaleFactor = EMOJI_SCALE_FACTOR;

        int newEmojiWidth = (int) (face.getWidth() * scaleFactor);
        int newEmojiHeight = (int) (emoji.getHeight() *
                newEmojiWidth / emoji.getWidth() * scaleFactor);
        ...
        float emojiPositionX =
                (face.getPosition().x + face.getWidth() / 2) - emoji.getWidth() / 2;
        float emojiPositionY =
                (face.getPosition().y + face.getHeight() / 2) - emoji.getHeight() / 3;
